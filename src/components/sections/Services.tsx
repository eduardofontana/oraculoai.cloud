...
import { Bot, Brain, Lightbulb, ShieldCheck, Globe, LayoutGrid, Check, Sparkles } from "lucide-react"
  ...
  return (
    ...
    >
      <Card hover className="h-full group">
        <div className={cn(
          "w-14 h-14 rounded-2xl flex items-center justify-center mb-5 bg-gradient-to-br transition-transform duration-300 group-hover:scale-110",
          gradient
        )}>
...
function PricingCard({ plan, index }: { plan: (typeof SERVICES.pricing.plans)[number]; index: number }) {
  return (
    ...
    >
      {plan.highlight && (
        <div className="absolute -top-3 left-1/2 -translate-x-1/2 bg-gradient-to-r from-[#7B4DFF] to-[#6D28D9] text-white px-4 py-1 rounded-full text-xs font-semibold">
          Mais Popular
        </div>
      )}
      <div className="mb-6">
      ...
      <a
        href={plan.highlight ? "/#diagnostico" : "/#contato"}
        className={cn(
          "block w-full text-center py-3.5 rounded-xl font-semibold text-sm transition-all duration-300",
          plan.highlight
            ? "bg-[#7B4DFF] hover:bg-[#6D28D9] text-white shadow-lg shadow-purple-500/25"
